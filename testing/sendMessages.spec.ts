...
const createProducerResponse = (topicName: any, partition: any) => ({
  topics: [
    {
      topicName,
      partitions: [
...
describe('Producer > sendMessages', () => {
  ...
  beforeEach(() => {
    ...
    brokers = {
      ...
      1: {
        nodeId: 1,
        produce: jest.fn(() => createProducerResponse(topic, 0)),
      },
      // @ts-expect-error ts-migrate(2304) FIXME: Cannot find name 'jest'.
      2: {
        nodeId: 2,
        produce: jest.fn(() => createProducerResponse(topic, 1)),
      },
      // @ts-expect-error ts-migrate(2304) FIXME: Cannot find name 'jest'.
      3: {
        nodeId: 3,
        produce: jest.fn(() => createProducerResponse(topic, 2)),
      },
      // @ts-expect-error ts-migrate(2304) FIXME: Cannot find name 'jest'.
      4: {
        nodeId: 4,
    ...
    cluster = {
      ...
      // @ts-expect-error ts-migrate(2304) FIXME: Cannot find name 'jest'.
      findBroker: jest.fn(({ nodeId }: any) => brokers[nodeId]),
    ...
    require('./groupMessagesPerPartition').mockImplementation(
      () => messagesPerPartition
    );
  });
      ...
      brokers[1].produce
        .mockImplementationOnce(() => {
          throw createErrorFromCode(
            errorCodes.find(({ type }: any) => type === errorType).code
          );
        })
        .mockImplementationOnce(() => createProducerResponse(topic, 0));
  ...
  test('does not re-produce messages to brokers that are no longer leaders after metadata refresh', async () => {
    ...
    brokers[2].produce
      .mockImplementationOnce(() => {
        throw createErrorFromCode(
          errorCodes.find(
            ({ type }: any) => type === 'NOT_LEADER_FOR_PARTITION'
          ).code
        );
      })
      .mockImplementationOnce(() => createProducerResponse(topic, 0));
  ...
  test('retrieves sequence information from the transaction manager and updates', async () => {
    ...
    // @ts-expect-error ts-migrate(2304) FIXME: Cannot find name 'expect'.
    expect(
      brokers[1].produce.mock.calls[0][0].topicData[0].partitions[0]
    ).toHaveProperty('firstSequence', 5);
    // @ts-expect-error ts-migrate(2304) FIXME: Cannot find name 'expect'.
    expect(
      brokers[2].produce.mock.calls[0][0].topicData[0].partitions[0]
    ).toHaveProperty('firstSequence', 5);
    // @ts-expect-error ts-migrate(2304) FIXME: Cannot find name 'expect'.
    expect(
      brokers[3].produce.mock.calls[0][0].topicData[0].partitions[0]
    ).toHaveProperty('firstSequence', 5);
  ...
  test('adds partitions to the transaction if transactional', async () => {
    ...
    // @ts-expect-error ts-migrate(2304) FIXME: Cannot find name 'expect'.
    expect(eosManager.addPartitionsToTransaction).toHaveBeenCalledTimes(
      numTargetBrokers
    );
    // @ts-expect-error ts-migrate(2304) FIXME: Cannot find name 'expect'.
