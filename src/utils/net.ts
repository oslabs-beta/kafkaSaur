...
import stream from 'https://deno.land/std@0.110.0/node/stream.ts';
// import debug from 'internal/util/debuglog').debuglog('net', (fn) => {
//   debug = fn;
// });


import { isIP, isIPv4, isIPv6, normalizedArgsSymbol, makeSyncWrite } from 'internal/net';
...
import { writevGeneric, writeGeneric, onStreamRead, kAfterAsyncWrite, kHandle, kUpdateTimer, setStreamTimeout, kBuffer, kBufferCb, kBufferGen } from 'internal/stream_base_commons';
const {
  codes: {
...
import { validateAbortSignal, validateFunction, validateInt32, validateNumber, validatePort, validateString } from 'internal/validators';
const kLastWriteQueueSize = Symbol('lastWriteQueueSize');
import { DTRACE_NET_SERVER_CONNECTION, DTRACE_NET_STREAM_END } from 'internal/dtrace';

// Lazy loaded to improve startup performance.
let cluster: any;
let dns: any;
let BlockList: any;
let SocketAddress: any;

import { clearTimeout } from 'timers';
import { kTimeout } from 'internal/timers';
...
function getFlags(ipv6Only : any) {
  return ipv6Only === true ? TCPConstants.UV_TCP_IPV6ONLY : 0;
}

function createHandle(fd:any, is_server:any) {
...
function getNewAsyncId(handle: any) {
...
function createServer(options: any, connectionListener: any) {
  return new Server(options: any, connectionListener: any): any;
}
...
function connect(...args: any[]) {
  ...
  const socket = new Socket(option)//must be written as a class to use the new keyword ;
...
function initSocketHandle(self: any) {
  self._undestroy();
  self._sockname = null;
...
function Socket(options: any): any {
...
function Server(options: any, connectionListener: any): any {
