...
describe('Protocol > Encoder', () => {
  ...
  const encodeDouble = (number: any) => new Encoder().writeDouble(number).buffer
  const decodeDouble = (buffer: any) => new Decoder(buffer).readDouble()

  const ustring = (string: any) => new Encoder().writeUVarIntString(string).buffer
  const decodeUString = (buffer: any) => new Decoder(buffer).readUVarIntString()
