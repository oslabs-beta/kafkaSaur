...
const indexByPartition = (array: any) => array.reduce(
  (obj: any, {
    partition,
    ...props
...
export default ({
    ...
    return retrier(async (bail: any, retryCount: any, retryTime: any) => {
      try {
        ...
        if (e instanceof KafkaJSAggregateError) {
          if (e.errors.every((error: any) => error.type === 'TOPIC_ALREADY_EXISTS')) {
            return false
          }
        }
  ...
  const fetchOffsets = async ({
    ...
    const result = consumerOffsets.map(({
      topic,
      partitions
    }: any) => {
  ...
  const deleteTopicRecords = async ({
    ...
    partitions.forEach(({
      ...
      const { low } = topicOffsets.find((p: any) => p.partition === partition) || {
        high: undefined,
        low: undefined,
      }
      ...
      try {
        ...
        if (
          ...
          e.partitions.some(
            ({
              error
            }: any) => staleMetadata(error) || error.name === 'KafkaJSMetadataNotLoaded'
          )
        ) {
          await cluster.refreshMetadata()
        }
